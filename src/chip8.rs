...
impl Chip8 {
    ...
    fn execute_instruction(&mut self, op: u16) -> Result<Chip8Event, Error> {
        #[cfg(feature = "trace")]
        println!("{:?}", self);
        let [ub, lb] = op.to_be_bytes();
        ...
        match op {
            ...
            0x8000..=0x8FFF => match n {
                ...
                _ => return Err(CPUError::UnknownOpcode(op, self.pc).into()),
            },
            //9xy0 SNE Vx, Vy
            0x9000..=0x9FF0 if op & 0xF == 0 => {
                if vx != vy {
                    return Ok(Chip8Event::SkipNextInstruction);
                }
            }
            //Annn LD I, addr
            0xA000..=0xAFFF => self.i = op & 0x0FFF,
            //Bnnn JP V0, addr
            0xB000..=0xBFFF => {
                self.pc = (op & 0x0FFF) + self.reg[0x0] as u16;
                return Ok(Chip8Event::DoNotIncrementPC);
            }
            //Cnnn RND Vx, byte
            0xC000..=0xCFFF => {
            ...
            _ => {
                return Err(CPUError::UnknownOpcode(op, self.pc).into());
            }
        }
